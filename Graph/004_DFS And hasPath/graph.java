...
public class graph {
    ...
    // O(E)
    public static int dfs(ArrayList<Edge>[] gp,
            boolean[] vis, int src) {
        vis[src] = true;
        int count = 1;
        for (Edge e : gp[src]) {
            if (!vis[e.v]) {
                count += dfs(gp, vis, e.v);
            }
        }
    ...
    public static int allComponents(ArrayList<Edge>[] gp,
            boolean[] vis) {
        int noOfComponent = 0;
        for (int i = 1; i < vis.length; i++) {
            if (!vis[i]) {
                noOfComponent++;
                int noOfVtx = dfs(gp, vis, i);
                System.out.println("Component: " + noOfComponent + " Total Vertex/Population: " + noOfVtx);
            }
        }
